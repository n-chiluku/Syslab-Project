...
def show_extraction_details(file_menu):
   ...
   # tk.Label(text=" | ", bg='light green', font=("Georgia bold", 10), fg='light green').grid(row=0, column=0, sticky=tk.W+tk.E+tk.N+tk.S) # highest padding
      ...
      for line in book_array:
         if line[0:1] == 'd':    #it's dialogue
            if line[2:line.find('-')-1] == characters[0]:     #speaker 1
               res = tts.synthesize(line[line.find('-'):], accept='audio/mp3', voice='en-US_HenryV3Voice').get_result() #selecting the audio format and voice
            ...
            res = tts.synthesize(line[2:], accept='audio/mp3', voice='en-US_AllisonV3Voice').get_result() #selecting the audio format and voice
            audio_file.write(res.content) #writing the content from text file to a audio file
   # os.open('audioBook.mp3', os.O_CREAT)
...
#    p = vlc.MediaPlayer(audio_file_name)
#    p.play()
...
#    pygame.mixer.music.play()
  ...
  #  PLAYERPATH = "C:/Users/1504522/Desktop/Syslab Project" + audio_file_name
#    subprocess.call([PLAYERPATH])
  
   # webbrowser.open(audio_file_name)
# 
#    os.system("TASKKILL /im mp3.exe /f")
   
   # os.close('audioBook.mp3')
