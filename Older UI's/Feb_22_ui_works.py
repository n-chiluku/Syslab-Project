...
def make_audiobook(characters):
  ...
  #  audio_file_name = 'audiobook.mp3'
   ...
   audio_file_name = book_title + '_audio.mp3'
   print(audio_file_name)
  #  for x in book_array: print(x)
   
   with open(path_name + audio_file_name, 'wb') as audio_file:  # works for two characters rn
      for line in book_array:
         if line[0:1] == 'd':    #it's dialogue
            ...
            else:    # speaker 2 (if there's more than 1 speaker, it's all of them
   ...
   print("fuil,", path_name + audio_file_name)   
...
def open_Audio_real(file_menu, characters): #need to pass audio file name now 
   path_name = 'C:/Users/1504522/Desktop/Syslab Project/Audiobooks/'
 #   audio_file_name = filename + '_audio.mp3'
   audio_file_name = book_title + '_audio.mp3'

   # if file doesnt exist:  
   
   if(len(find_files(audio_file_name,"C:")) == 0):
      make_audiobook(characters)
      print("fresh", audio_file_name)
      if (len(find_files(audio_file_name,"C:"))) == 0: print("no!")
   else:
...
#  
   ...
   os.startfile(path_name + audio_file_name)  #works
